...
delete(Bucket, Selector) ->
	FilesColl = bson:list_to_atom_ex(atom_to_list(Bucket) ++ ".files"),
	ChunksColl = bson:list_to_atom_ex(atom_to_list(Bucket) ++ ".chunks"),
...
insert(Coll, ObjectId, N, Data) when size(Data) =< ?CHUNK_SIZE ->
  %%php存储到mongo中的GridFS的chunks的data似乎包含了4字节的长度，BUG？？还是兼容一下他吧
%	mongo:insert(Coll, {'files_id', ObjectId, data, {bin, bin, Data}, n, N});
  BDataLen = binary:encode_unsigned(size(Data), little),
  Pad0Len = 8 * (4 - size(BDataLen)),
	mongo:insert(Coll, {'files_id', ObjectId, data, {bin, bin_old, <<BDataLen/binary, 0:Pad0Len, Data/binary>>}, n, N});
...
%	mongo:insert(Coll, {'files_id', ObjectId, data, {bin, bin, <<Data1:(?CHUNK_SIZE*8)>>}, n, N}),
  mongo:insert(Coll, {'files_id', ObjectId, data, {bin, bin_old, <<0:8, 0:8, 4:8, 0:8, Data1:(?CHUNK_SIZE*8)>>}, n, N}),
...
%			mongo:insert(ChunksColl, {'files_id', ObjectId, data, {bin, bin, Data}, n, N}),
      BDataLen = binary:encode_unsigned(size(Data), little),
      Pad0Len = 8 * (4 - size(BDataLen)),
      mongo:insert(ChunksColl, {'files_id', ObjectId, data, {bin, bin_old, <<BDataLen/binary, 0:Pad0Len, Data/binary>>}, n, N}),
