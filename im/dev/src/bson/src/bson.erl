...
-export ([list_to_atom_ex/1, to_bin/1, dehex/1, bin_to_hexstr/1]).
...
lookup (Label, Doc) ->
	...
	case length (Parts) of
		1 ->
			case find (list_to_atom_ex (hd (Parts)), Doc) of
						{Index} -> {element (Index * 2 + 2, Doc)};
						{} -> {} end;
		_ ->
			case find (list_to_atom_ex (hd (Parts)), Doc) of
				{Index} -> lookup (list_to_atom_ex (string:join (tl (Parts), ".")), element (Index * 2 + 2, Doc));
				{} -> {} end
	end.
...
update (Label, Value, Document) ->
	...
	case length (Parts) of
		1 ->
			case find (list_to_atom_ex (hd (Parts)), Document) of
		...
		_ ->
			case find (list_to_atom_ex (hd (Parts)), Document) of
				{Index} -> setelement (Index * 2 + 2, Document, update (list_to_atom_ex (string:join (tl (Parts), ".")), Value, element (Index * 2 + 2, Document)));
				{} -> Doc = erlang:append_element (Document, list_to_atom_ex (hd (Parts))),
						erlang:append_element (Doc, update (list_to_atom_ex (string:join (tl (Parts), ".")), Value, {})) end
	end.
...
% An Erlang string() is a list of unicode characters (codepoints), but this list must be converted to utf-8 binary for use in Bson. Call utf8/1 to do this, or encode pure ascii literals directly as `<<"abc">>' and non-pure ascii literals as `<<"a遚"/utf8>>'.
...
%list_to_atom atom所占内存不会被释放，直接使用可能会有内存问题 
list_to_atom_ex(QList) when is_list(QList) ->
  case catch list_to_existing_atom(QList) of
    {'EXIT', _Reason} ->
        list_to_atom(QList);
    Atom ->
        Atom
