...
handle_info({route, From, To, {xmlelement, "iq", _, _} = Packet}, State) ->
    ...
    case catch process_iq(From, To, IQ, State) of
...
%% disco#info request
process_iq(_, _, #iq{type = get, xmlns = ?NS_DISCO_INFO, lang = Lang} = IQ,
...
%% disco#items request
process_iq(_, _, #iq{type = get, xmlns = ?NS_DISCO_ITEMS} = IQ, _) ->
    IQ#iq{type = result, sub_el =
	  [{xmlelement, "query", [{"xmlns", ?NS_DISCO_ITEMS}], []}]};

%% vCard request
process_iq(_, _, #iq{type = get, xmlns = ?NS_VCARD, lang = Lang} = IQ, _) ->
    IQ#iq{type = result, sub_el =
	  [{xmlelement, "vCard", [{"xmlns", ?NS_VCARD}], iq_vcard(Lang)}]};

%% bytestreams info request
process_iq(JID, _, #iq{type = get, sub_el = SubEl, xmlns = ?NS_BYTESTREAMS} = IQ,
...
process_iq(InitiatorJID, To, #iq{type = set, sub_el = SubEl, xmlns = ?NS_BYTESTREAMS} = IQ,
	   #state{acl = ACL, serverhost = ServerHost}) ->
    SelfNodeStr = atom_to_list(node()),
	  ALfileproxy_node = ejabberdex_odbc_query:get_fileproxy_node(jlib:jid_to_string(InitiatorJID)),
	  case ALfileproxy_node of
	  [Afileproxy_node|_] when element(2, Afileproxy_node) /= SelfNodeStr ->
	    ejabberdex_cluster_router:route(element(2, Afileproxy_node), InitiatorJID#jid.luser ++ "@" ++ InitiatorJID#jid.lserver, InitiatorJID#jid.resource, {InitiatorJID, To, jlib:iq_to_xml(IQ#iq{sub_el = [SubEl]})}),
      ignore;
    _ ->	  
...
process_iq(_, _, #iq{type=Type, sub_el=SubEl} = IQ, _) when Type==get; Type==set ->
