...
-include("../../public_key-0.13/include/OTP-PUB-KEY.hrl").
...
wait_for_stream({xmlstreamstart, _Name, Attrs}, StateData) ->
	...
	?NS_STREAM ->
	    ...
	    case lists:member(Server, ?MYHOSTS) orelse ejabberd_config:is_register_host(Server) of
		true ->
...
terminate(_Reason, StateName, StateData) ->
	...
	session_established ->
		...
		_ ->
		    ...
		    del_global_session(StateData#state.user, StateData#state.server, StateData#state.resource)
	    end,
	    bounce_messages();
	_ ->
	    ok
...
%%----------------------------------------------------------------------
%% wefafa license check
-record(license_fafa, {serial="", enterprise="", clusternodenum="", type="", expiration="", registrationcode="", signature=""}).
...
%% @spec () -> ok
check_license() ->
	Aregistration_code = create_registration_code(),

	%%检查lic文件是否存在
	ALicFilePath = code:root_dir() ++ "/conf/lic.txt",
	ALicIODevice = case file:open(ALicFilePath, [read]) of
		{ok, IDev} ->
      		IDev;
    	_ ->
      		% ?PRINT("~p~n  ", [Err]),
      		halt_invaid_lic(Aregistration_code)
	end, 

	%%读取内容
	Alicense_fafa = parse_licfile(ALicIODevice, #license_fafa{}),
	Bserial = list_to_binary(Alicense_fafa#license_fafa.serial), 
	Benterprise = list_to_binary(Alicense_fafa#license_fafa.enterprise), 
	Bclusternodenum = list_to_binary(Alicense_fafa#license_fafa.clusternodenum), 
	Btype = list_to_binary(Alicense_fafa#license_fafa.type), 
	Bexpiration = list_to_binary(Alicense_fafa#license_fafa.expiration), 
	Bregistrationcode = list_to_binary(Alicense_fafa#license_fafa.registrationcode), 
	Bsignature = bson:to_bin(Alicense_fafa#license_fafa.signature),

	%%通过公钥验证文件有效性
	Awillverifydata = <<Bserial/binary, 
					"!", Benterprise/binary, 
					"@", Bclusternodenum/binary, 
					"#", Bregistrationcode/binary,
					"$", Btype/binary,
					"%", Bexpiration/binary>>,
	Acertdata = <<"-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----">>,
	{'Certificate', Acert_der_encoded, _} = hd(public_key:pem_decode(Acertdata)),
	Acert = public_key:pkix_decode_cert(Acert_der_encoded, otp),
	Arsa_public_key = ((Acert#'OTPCertificate'.tbsCertificate)#'OTPTBSCertificate'.subjectPublicKeyInfo)#'OTPSubjectPublicKeyInfo'.subjectPublicKey,
	case public_key:verify(Awillverifydata, 'sha', Bsignature, Arsa_public_key) of
		true ->
			ok;
		_ ->
      		halt_invaid_lic(Aregistration_code) 
	...
	%%计算本机注册码，并比较lic文件中的注册码
	case Alicense_fafa#license_fafa.registrationcode of
		Aregistration_code ->
			ok;
		_ ->
      		halt_invaid_lic(Aregistration_code) 
	...
	%%检查有效期、节点数等信息
	{AtodayY, AtodayM, AtodayD} = date(),
	{AexpirationY, AexpirationM, AexpirationD} = ejabberdex_odbc_query:list_to_date(Alicense_fafa#license_fafa.expiration),
	case AexpirationY < AtodayY
		orelse (AexpirationY == AtodayY andalso AexpirationM < AtodayM)
		orelse (AexpirationY == AtodayY andalso AexpirationM == AtodayM andalso AexpirationD < AtodayD) of
		true ->
			% ?PRINT("您的授权文件已过期！\n", []),
			?ERROR_MSG("您的授权文件已过期！\n", []),
			halt_invaid_lic(Aregistration_code);
	...
	Aclusternodenum = list_to_integer(Alicense_fafa#license_fafa.clusternodenum),
	Anowrun_clusternodenum = length(ejabberdex_cluster_router:get_cluster_node()),
	case Aclusternodenum < Anowrun_clusternodenum of
		true ->
			?ERROR_MSG("您的集群节点数量已超过授权！\n", []),
			timer:sleep(300),
			halt(1);
		_ ->
      		ok 
...
create_registration_code() ->
	ALocalIDData = os:cmd("dmidecode | grep ID ; ifconfig eth0 | head -1 | awk '{print $5}' ; dmidecode | grep Serial;"),
	bson:bin_to_hexstr(crypto:md5(ALocalIDData)).

%% 无效证书，终止程序
halt_invaid_lic(Aregistration_code) ->
	% ?PRINT("--------------------------------------------------------------------\n", []),
	% ?PRINT("\n", []),
	% ?PRINT("您的授权文件不正确！请重新申请授权！注册码：~s\n", [Aregistration_code]),
	% ?PRINT("your license file is invaild! please apply again! RegistrationCode:~s\n", [Aregistration_code]),
	% ?PRINT("\n", []),
	% ?PRINT("--------------------------------------------------------------------\n", []),
	?ERROR_MSG("
--------------------------------------------------------------------

您的授权文件不正确！请重新申请授权！注册码：~s
your license file is invaild! please apply again! RegistrationCode:~s

--------------------------------------------------------------------\n", 
		[Aregistration_code, Aregistration_code]),
	timer:sleep(300),
	halt(1),
	ok.

%% 解析LIC文件
%% @spec (io_device(), #license_fafa{}) -> #license_fafa{}
parse_licfile(QLicIODevice, Qlicense_fafa) ->
	case file:read_line(QLicIODevice) of
		{ok, Data} ->
			case re:split(Data,"=",[{return,list}]) of
				[_Value] -> % ignore
					parse_licfile(QLicIODevice, Qlicense_fafa);
				[""|_LineValues] -> % line begins with "=", ignore
					parse_licfile(QLicIODevice, Qlicense_fafa);
				[ValueName|LineValues] -> % yeehaw, got a line!
					ALineValues = string:strip(lists:flatten(LineValues), both, $\n),
					case ValueName of
						"Serial" -> parse_licfile(QLicIODevice, Qlicense_fafa#license_fafa{serial = ALineValues});
						"Enterprise" -> parse_licfile(QLicIODevice, Qlicense_fafa#license_fafa{enterprise = ALineValues});
						"ClusterNodeNum" -> parse_licfile(QLicIODevice, Qlicense_fafa#license_fafa{clusternodenum = ALineValues});
						"Type" -> parse_licfile(QLicIODevice, Qlicense_fafa#license_fafa{type = ALineValues});
						"Expiration" -> parse_licfile(QLicIODevice, Qlicense_fafa#license_fafa{expiration = ALineValues});
						"RegistrationCode" -> parse_licfile(QLicIODevice, Qlicense_fafa#license_fafa{registrationcode = ALineValues});
						"Signature" -> parse_licfile(QLicIODevice, Qlicense_fafa#license_fafa{signature = ALineValues})							
					end					
			end;
		eof ->
			Qlicense_fafa;
