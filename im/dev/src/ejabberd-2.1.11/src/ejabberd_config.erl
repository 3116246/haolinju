...
-include("../../kernel-2.14.5/include/file.hrl").
...
%% 判断Host是否已注册子域，若未在缓存配置中加入，则加入
is_register_host(Host) ->
  Aeno = hd(string:tokens(Host, ".")),
  ALenterprise_reg = ejabberdex_odbc_query:get_enterprise_by_eno(Aeno),
  Are = case ALenterprise_reg of
    [] ->
      false;
    _ ->
      re_load_file(),
      MainDomain = func_utils:getlocalserver(), 
      lists:foreach(fun(ItemHost) ->
      	  case mnesia:dirty_read(route,ItemHost) of []->
                      case ejabberd_config:get_local_option({modules, ItemHost}) of
                  		  undefined ->
                  		      ok;
                  		  Modules1 ->
                  		      lists:foreach(
                  						fun({Module1, Args}) ->
                  							gen_mod:start_module(ItemHost, Module1, Args)
                  						end, Modules1)
                  	  end,
                  	  
      	              [Mr]=mnesia:dirty_read(route, MainDomain),
      	      		    F = fun() ->
      									mnesia:write(#route{domain = ItemHost,
      											    pid = Mr#route.pid,
      											    local_hint = Mr#route.local_hint})
      								end,
...
re_load_file() ->
  File = get_ejabberd_config_path(),
  Terms = get_plain_terms_file(File),
  State = lists:foldl(fun search_hosts_odbc/2, #state{}, Terms),
  Terms_macros = replace_macros(Terms),
  Res = lists:foldl(fun process_term/2, State, Terms_macros),
  set_opts(Res).  

search_hosts_odbc(Term, State) ->
    case Term of
	{host, Host} ->
	    if
		State#state.hosts == [] ->
		    ALenterprise_reg = ejabberdex_odbc_query:get_enterprise(),
		    ALHost = [A#enterprise_reg.subdomain || A <- ALenterprise_reg, A#enterprise_reg.subdomain /= Host],
		    add_hosts_to_option([Host | ALHost], State);
		true ->
		    ?ERROR_MSG("Can't load config file: "
			       "too many hosts definitions", []),
		    exit("too many hosts definitions")
	...
	{hosts, Hosts} ->
		...
		State#state.hosts == [] ->
		    ALenterprise_reg = ejabberdex_odbc_query:get_enterprise(),
		    ALHost = [A#enterprise_reg.subdomain || A <- ALenterprise_reg, not lists:member(A#enterprise_reg.subdomain, Hosts)],
		    add_hosts_to_option(Hosts++ALHost, State);
		true ->
		    ?ERROR_MSG("Can't load config file: "
			       "too many hosts definitions", []),
		    exit("too many hosts definitions")
