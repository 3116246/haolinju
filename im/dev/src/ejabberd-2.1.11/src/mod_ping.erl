...
handle_cast({iq_pong, JID, timeout}, State) ->
    ?DEBUG("iq_pong timeout ~p ~p~n", [State#state.timeout_action,JID]),
    Timers = del_timer(JID, State#state.timers),
  ...
  kill ->
    ...
    Pid when is_pid(Pid) ->
        ...
        mod_presence_ex_odbc:user_lose_connection(User, Server, Resource);
    _ ->
        ok
      end;
  _ ->
...
iq_ping(From, _To, #iq{type = Type, sub_el = SubEl} = IQ) ->
    ?DEBUG("iq_ping~n~p~n~p~n", [From,Type,SubEl]),
    case {Type, SubEl} of
