		...
		true ->
		    Query = lists:map(
			      fun(M) ->
				      ...
				      LServer = ejabberd_odbc:escape(
					    (M#offline_msg.to)#jid.lserver),
					 ...
					 jlib:timestamp_to_xml( 
				      ...
				      odbc_queries:add_spool_sql(LServer, Username, XML)
			      end, Msgs),
...
pop_offline_messages(_Ls, User, _Server) ->
    EUser = User#jid.luser,%%jlib:nodeprep(User),
    LServer = User#jid.lserver,%%jlib:nameprep(Server),
...
get_queue_length(Username, LServer) ->
    case catch ejabberd_odbc:sql_query(
			    ...
			    ["select count(*) from spool"
			     "  where username='", Username++"@"++LServer, "';"]) of
		   {selected, [_], [{SCount}]} ->
		       SCount;
		   _ ->
		       0
...
count_offline_messages(LUser, LServer) ->
    ...
    case catch odbc_queries:count_records_where(
		 LServer, "spool", "where username='" ++ Username++"@"++LServer ++ "'") of
        {selected, [_], [{Res}]} ->
            list_to_integer(Res);
        _ ->
            0
