...
-include("../include/web/ejabberd_http.hrl").
-include("../include/web/ejabberd_web_admin.hrl").
...
%				    odbc_queries:set_roster_version(ejabberd_odbc:escape(LUser), RosterVersion)
				    odbc_queries:set_roster_version(LServer, ejabberd_odbc:escape(LUser), RosterVersion)
				end),
				RosterVersion
			end;
		false ->
...
get_roster(LUser, LServer) ->
	...
	{selected, ["username", "jid", "nick", "subscription", "ask",
		    "askmessage", "server", "subscribe", "type","spell","photo","p_desc"],
...
item_to_xml(Item) ->
    Attrs1 = [{"jid", jlib:jid_to_string(Item#roster.jid)},
    		  {"spell",Item#roster.spell},
    		  {"photo",Item#roster.photo},
    		  {"p_desc",Item#roster.p_desc}],
	    ...
	    F = fun() ->
			{selected,
			 ["username", "jid", "nick", "subscription",
			  "ask", "askmessage", "server", "subscribe", "type","spell","photo","p_desc"],
...
%				true -> odbc_queries:set_roster_version(ejabberd_odbc:escape(LUser), sha:sha(term_to_binary(now())));
				true -> odbc_queries:set_roster_version(LServer, ejabberd_odbc:escape(LUser), sha:sha(term_to_binary(now())));
				false -> ok
			end,
			{Item, Item3}
		end,
...
process_item_set_t(LUser, LServer, {xmlelement, _Name, Attrs, Els}) ->
	...
	_ ->
	    ...
	    Username = ejabberd_odbc:escape(LUser),
	    LLServer = ejabberd_odbc:escape(LServer),
	    SJID = ejabberd_odbc:escape(jlib:jid_to_string(LJID)),
...
%		    odbc_queries:del_roster_sql(Username, SJID);
		    odbc_queries:del_roster_sql(LLServer, Username, SJID);
	        _ ->
	            ItemVals = record_to_string(Item1),
		    ItemGroups = groups_to_string(Item2),
%		    odbc_queries:update_roster_sql(Username, SJID, ItemVals, ItemGroups)
		    odbc_queries:update_roster_sql(LLServer, Username, SJID, ItemVals, ItemGroups)
	    end
    end;
...
raw_to_record(LServer, {UserServer, SJID, Nick, SSubscription, SAsk, SAskMessage,
			_SServer, _SSubscribe, _SType,Spell,Photo,Desc}) ->
...
%		    us = {User, LServer},
      UserJid = jlib:string_to_jid(UserServer),
	    #roster{usj = {UserJid#jid.luser, LServer, LJID},
		    us = {UserJid#jid.luser, LServer},
		    jid = LJID,
		    name = Nick,
...
record_to_string(#roster{us = {User, Server},
			 jid = JID,
			 name = Name,
			 subscription = Subscription,
			 ask = Ask,
...
%    [Username, SJID, Nick, SSubscription, SAsk, SAskMessage, "N", "", "item"].
    [Username++"@"++ejabberd_odbc:escape(Server), SJID, Nick, SSubscription, SAsk, SAskMessage, "N", "", "item"].

%groups_to_string(#roster{us = {User, _Server},
groups_to_string(#roster{us = {User, Server},
			 jid = JID,
			 groups = Groups}) ->
%    Username = ejabberd_odbc:escape(User),
    Username = ejabberd_odbc:escape(User)++"@"++ejabberd_odbc:escape(Server),
    SJID = ejabberd_odbc:escape(jlib:jid_to_string(jlib:jid_tolower(JID))),
