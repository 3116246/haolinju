...
update_info_ex() ->
  update_info_ex(bson),
  update_info_ex(mod_ejabberdex_init),
  update_info_ex(mongo),
  update_info_ex(mysql_auth),
  ok.
update_info_ex(M) ->
  Dir = filename:dirname(code:which(M)),
    case file:list_dir(Dir) of
	{ok, Files} ->
	    update_info_ex(Dir, Files);
	{error, Reason} ->
	    {error, Reason}
    end.
%%--------------------------------------------------------------------
...
update_info_ex(_Dir, Files) ->
  Beams = lists:sort(get_beams(Files)),
  UpdatedBeams = get_updated_beams(Beams),
  [code:purge(M) || M <- UpdatedBeams],
  [code:load_file(M) || M <- UpdatedBeams].
