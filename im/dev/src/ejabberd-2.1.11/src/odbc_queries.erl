...
set_last_t(LServer, Username, Seconds, State) ->
    update(LServer, "last", ["username", "seconds", "state"],
	   [Username++"@"++LServer, Seconds, State],
...
get_and_del_spool_msg_t(LServer, Username) ->
    F = fun() ->
		Result = ejabberd_odbc:sql_query_t(
			   ["select username, xml from spool where username='", Username++"@"++LServer, "'"
			    "  order by seq;"]),
		ejabberd_odbc:sql_query_t(
...
get_roster(LServer, Username) ->
    ejabberd_odbc:sql_query(
      LServer,
      ["select a.username, a.jid, a.nick, a.subscription, a.ask, "
       "a.askmessage, a.server, a.subscribe, a.type,ifnull(b.spell,'') spell,ifnull(b.photo,'') photo,ifnull(b.p_desc,'') p_desc from rosterusers a left join im_employee b"
       " on a.jid=b.loginname where a.username='", Username++"@"++LServer, "'"]).
...
get_roster_by_jid(LServer, Username, SJID) ->
    ejabberd_odbc:sql_query_t(
    ["select a.username, a.jid, a.nick, a.subscription, "
     "a.ask, a.askmessage, a.server, a.subscribe, a.type,ifnull(b.spell,'') spell,ifnull(b.photo,'') photo,ifnull(b.p_desc,'') p_desc from rosterusers a left join im_employee b"
     " on a.jid=b.loginname where a.username='", Username++"@"++LServer, "' "
     " and a.jid='", SJID, "';"]).
...
update_roster(LServer, Username, SJID, ItemVals, ItemGroups) ->
    update_t("rosterusers",
	     ...
	     ItemVals,
	     ["username='", Username++"@"++LServer, "' and jid='", SJID, "'"]),
    ejabberd_odbc:sql_query_t(
      ["delete from rostergroups "
       "      where username='", Username++"@"++LServer, "' "
       "        and jid='", SJID, "';"]),
...
set_private_data(LServer, Username, LXMLNS, SData) ->
    update_t("private_storage",
	     ...
	     [Username++"@"++LServer, LXMLNS, SData], 
	     ["username='", Username++"@"++LServer, "' and namespace='", LXMLNS, "'"]).

set_private_data_sql(LServer, Username, LXMLNS, SData) ->
    [["delete from private_storage "
       "where username='", Username++"@"++LServer, "' and "
       "namespace='", LXMLNS, "';"],
      ["insert into private_storage(username, namespace, data) "
       "values ('", Username++"@"++LServer, "', '", LXMLNS, "', "
       "'", SData, "');"]].
...
del_user_private_storage(LServer, Username) ->
    ejabberd_odbc:sql_query(
      LServer,
      ["delete from private_storage where username='", Username++"@"++LServer, "';"]).
      ...
      fun() ->
	      ...
	      update_t("vcard_search",
			...
			"orgname", "lorgname", "orgunit", "lorgunit"],
		       [Username++"@"++LServer, LUsername++"@"++LServer, SFN, SLFN, SFamily, SLFamily,
...
get_privacy_list_id(LServer, Username, SName) ->
    ejabberd_odbc:sql_query(
      ...
      ["select id from privacy_list "
       "where username='", Username++"@"++LServer, "' and name='", SName, "';"]).
...
get_privacy_list_data(LServer, Username, SName) ->
    ejabberd_odbc:sql_query(
      ...
      ["select t, value, action, ord, match_all, match_iq, "
       ...
       "            username='", Username++"@"++LServer, "' and name='", SName, "') "
       "order by ord;"]).
...
set_default_privacy_list(LServer, Username, SName) ->
    update_t("privacy_default_list", ["username", "name"],
	     [Username++"@"++LServer, SName], ["username='", Username++"@"++LServer, "'"]).
...
del_privacy_lists(LServer, Server, Username) ->
    ejabberd_odbc:sql_query(
      LServer,
      ["delete from privacy_list where username='", Username++"@"++Server, "';"]),
    ejabberd_odbc:sql_query(
      LServer,
      ["delete from privacy_list_data where value='", Username++"@"++Server, "';"]),
    ejabberd_odbc:sql_query(
      LServer,
      ["delete from privacy_default_list where username='", Username++"@"++Server, "';"]).
...
get_roster_version(LServer, LUser) ->
	ejabberd_odbc:sql_query(LServer,
		["select version from roster_version where username = '", LUser++"@"++LServer, "'"]).
set_roster_version(LServer, LUser, Version) ->
	update_t("roster_version", ["username", "version"], [LUser++"@"++LServer, Version], ["username = '", LUser++"@"++LServer, "'"]).
-endif.
...
set_last_t(LServer, Username, Seconds, State) ->
    Result = ejabberd_odbc:sql_query(
		 LServer,
		 ["EXECUTE dbo.set_last '", Username++"@"++LServer, "', '", Seconds,
		  "', '", State, "'"]),
    {atomic, Result}.
...
set_password_t(LServer, Username, Pass) ->
    Result = ejabberd_odbc:sql_query(
      LServer,
      ["EXECUTE dbo.set_password '", Username++"@"++LServer, "', '", Pass, "'"]),
    {atomic, Result}.

add_user(LServer, Username, Pass) ->
    ejabberd_odbc:sql_query(
      LServer,
      ["EXECUTE dbo.add_user '", Username++"@"++LServer, "', '", Pass, "'"]).
...
add_spool_sql(LServer, Username, XML) ->
    ["EXECUTE dbo.add_spool '", Username++"@"++LServer, "' , '",XML,"'"].
...
get_roster_groups(LServer, Username, SJID) ->
    ejabberd_odbc:sql_query(
      LServer,
      ["EXECUTE dbo.get_roster_groups '", Username++"@"++LServer, "' , '", SJID, "'"]).
...
get_roster_by_jid(LServer, Username, SJID) ->
    ejabberd_odbc:sql_query(
      LServer,
      ["EXECUTE dbo.get_roster_by_jid '", Username++"@"++LServer, "' , '", SJID, "'"]).

get_rostergroup_by_jid(LServer, Username, SJID) ->
    ejabberd_odbc:sql_query(
      LServer,
      ["EXECUTE dbo.get_rostergroup_by_jid '", Username++"@"++LServer, "' , '", SJID, "'"]).

del_roster(LServer, Username, SJID) ->
    ejabberd_odbc:sql_query(
      LServer,
      ["EXECUTE dbo.del_roster '", Username++"@"++LServer, "', '", SJID, "'"]).

del_roster_sql(LServer, Username, SJID) ->
    ["EXECUTE dbo.del_roster '", Username++"@"++LServer, "', '", SJID, "'"].

update_roster(LServer, Username, SJID, ItemVals, ItemGroups) ->
    Query1 = ["EXECUTE dbo.del_roster '", Username++"@"++LServer, "', '", SJID, "' "],
    ...
    Query3 = ["EXECUTE dbo.del_roster_groups '", Username++"@"++LServer, "', '", SJID, "' "],
...
update_roster_sql(LServer, Username, SJID, ItemVals, ItemGroups) ->
    ["BEGIN TRANSACTION ",
     "EXECUTE dbo.del_roster_groups '", Username++"@"++LServer, "','", SJID, "' ",
...
get_subscription(LServer, Username, SJID) ->
    ejabberd_odbc:sql_query(
      LServer,
      ["EXECUTE dbo.get_subscription '", Username++"@"++LServer, "' , '", SJID, "'"]).
...
set_private_data_sql(LServer, Username, LXMLNS, SData) ->
    ["EXECUTE dbo.set_private_data '", Username++"@"++LServer, "' , '", LXMLNS, "' , '", SData, "'"].

get_private_data(LServer, Username, LXMLNS) ->
    ejabberd_odbc:sql_query(
        LServer,
        ["EXECUTE dbo.get_private_data '", Username++"@"++LServer, "' , '", LXMLNS, "'"]).
...
set_vcard(LServer, LUsername, SBDay, SCTRY, SEMail, SFN, SFamily, SGiven,
    ...
    ejabberd_odbc:sql_query(
      LServer,
      ["EXECUTE dbo.set_vcard '", SVCARD, "' , '", Username++"@"++LServer, "' , '", LUsername, "' , '",
...
get_privacy_list_id(LServer, Username, SName) ->
    ejabberd_odbc:sql_query(
      LServer,
      ["EXECUTE dbo.get_privacy_list_id '", Username++"@"++LServer, "' , '", SName, "'"]).
...
get_privacy_list_data(LServer, Username, SName) ->
    ejabberd_odbc:sql_query(
      LServer,
      ["EXECUTE dbo.get_privacy_list_data '", Username++"@"++LServer, "' , '", SName, "'"]).
...
set_default_privacy_list(LServer, Username, SName) ->
    ejabberd_odbc:sql_query_t(
      ["EXECUTE dbo.set_default_privacy_list '", Username++"@"++LServer, "' , '", SName, "'"]).
...
remove_privacy_list(LServer, Username, SName) ->
    ejabberd_odbc:sql_query_t(
      ["EXECUTE dbo.remove_privacy_list '", Username++"@"++LServer, "' , '", SName, "'"]).

add_privacy_list(LServer, Username, SName) ->
    ejabberd_odbc:sql_query_t(
      ["EXECUTE dbo.add_privacy_list '", Username++"@"++LServer, "' , '", SName, "'"]).
...
del_privacy_lists(LServer, Server, Username) ->
	ejabberd_odbc:sql_query(
      LServer,
      ["EXECUTE dbo.del_privacy_lists @Server='", Server ,"' @username='", Username++"@"++Server, "'"]).
...
set_roster_version(LServer, Username, Version) ->
    %% This function doesn't know the vhost, so we hope it's the first one defined:
%    LServer = ?MYNAME,
    ejabberd_odbc:sql_query(
      LServer,
      ["EXECUTE dbo.set_roster_version '", Username++"@"++LServer, "', '", Version, "'"]).
-endif.
