...
process_local_iq(From, To, #iq{sub_el = SubEl} = IQ) ->
	...
	true ->
        ...
        "groupchat" -> save_groupchat(From, To, IQ);
		_ -> IQ#iq{type = error,
...
%%%   <groupchat xmlns='http://im.fafacn.com/namespace/group' groupid='123456'>
%%%     <text>this is a test.</text>
%%%   </groupchat>
%%% </iq>
%%% 返回值 IQ节
save_groupchat(From, _To, #iq{sub_el = SubEl} = IQ) ->
    %%Operator = From#jid.luser ++ "@" ++ From#jid.lserver,    
    Qgroupid = xml:get_tag_attr_s("groupid", SubEl),
    To = jlib:make_jid(Qgroupid,From#jid.lserver,""),
    send_groupchat_message(From,To,SubEl)
.
...
send_groupchat_message(From, To, P) ->
