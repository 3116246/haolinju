...
add_log(Direction, LUser, LServer, LResource, JID, Packet) ->
    case parse_message(Packet) of
        {Type, Thread, Subject, Nick, Body,Groupid}  ->
            Proc = gen_mod:get_module_proc(LServer, ?PROCNAME),
            case Type of 
                "groupchat"->
                    gen_server:cast(
                     Proc, {addlog, "message", Direction, LUser, LServer, LResource, jlib:make_jid(Groupid,LServer,"groupchat"), Thread, Subject, Nick, Body});
                _->
                    gen_server:cast(
...
parse_message({xmlelement, _, _, [SubEl]}) ->
     case SubEl of 
        {xmlelement,"groupchat",_,_} ->
            Qgroupid = xml:get_tag_attr_s("groupid", SubEl),
            {"groupchat",
                         xml:get_path_s(SubEl, [{elem, "thread"}, cdata]),
                         xml:get_path_s(SubEl, [{elem, "subject"}, cdata]),
                         xml:get_path_s(SubEl, [{elem, "nickname"}, cdata]),
                         xml:get_path_s(SubEl, [{elem, "text"}, cdata]),
                         Qgroupid};
        _-> ""
    end;
parse_message(_) ->
