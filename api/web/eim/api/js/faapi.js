...
if (currentScriptSrc.indexOf("?") > -1) {
    ...
    for (var i = 0; i < parameters.length; i++) {
        ...
        if (paraName == "server") {
            ...
            Apis.put("Im-Server-https", s+":5281/http-bind");
			Apis.put("Im-Server-http", s+"/http-bind");
        }
...
LoadCSS.load = function(cssfile) {
    ...
    if (!isRef) {
        ...
        css.href = cssfile.indexOf("/") == -1 ? (LoadJs.domain + "/eim/component/css/" + cssfile) : cssfile;
        css.rel = 'stylesheet';
        css.type = 'text/css';
...
LoadJs.load = function(jsName) {
    ...
    if (!isRef) {
        ...
        oScript.src = (jsName.indexOf("/") == -1 ? (LoadJs.domain + "/eim/api/js/" + jsName) : (jsName)) + "?t=" + (new Date().getMonth() + "" + new Date().getDate());
