...
var MessageUtil = {
    ...
    //判断msg中是否有url链接，有则替换成固定样式<a style="padding: 2px; color: #0078B4;" target="_blank" title="http://j.map.baidu.com/RavEu" href="http://j.map.baidu.com/RavEu"><img src="/bundles/justsybase/images/face/../link16.png"> 链接地址</a>
    ...
    ParseFaces: function (text) {
        ...
        if (text.length > 7 && text.substring(0, 7) == "Web")
...
LoadCSS.load(WebIM_domain + "/eim/chat/css/FaFaChatWindow.css");
LoadCSS.load(WebIM_domain + "/eim/chat/css/chatmsg.css");
LoadJs.load(WebIM_domain + "/eim/chat/js/editor/xheditor-1.1.13-zh-cn.min.js");
LoadJs.load(WebIM_domain + "/eim/chat/js/imgZoom.js");
...
FaFaChatFileUpload.Init = function () {
    var callUrl = window.location.protocol + "//" + window.location.host + (window.location.port == "" ? "" : ":" + window.location.host)+"/webim_file_upload.html";
...
FaFaChatFileUpload.AsynResult = function (r) {
    ...
    try {
        ...
        if (obj.msg == "1") {
                      //判断当前发送的是否手机客户端
                      if(FaFaChatFileUpload.sendToJID.indexOf("Android")>-1 || FaFaChatFileUpload.sendToJID.indexOf("Iphone")>-1)
                    {
                          //发送离线文件通知
...
FaFaChatFileUpload.Request = function () {
    ...
    {
       //发送一个请求iq
       FaFaMessage.SendFileRequest(FaFaChatWin.owner.GetJid("Web"), FaFaChatFileUpload.sendToJID, fn, fileId, FaFaChatWin.owner.name == "" ? null : FaFaChatWin.owner.name);
    }
...
FaFaChatFileUpload.Cancel = function (fn) {
    //发送文件取消iq    
    FaFaMessage.CancelSendFileRequest(FaFaChatWin.owner.GetJid("Web"), FaFaChatWin.active.GetJid(), fn);
    FaFaChatWin.isSendFile = false;
    $("#xhe0_Tool [cmd='sendFile']").attr("title", "文件发送");
    FaFaChatFileUpload.sendingId = "";
...
FaFaChatFileUpload.Reject = function (fn, fileid) {
    ...
    FaFaMessage.RejectFile(FaFaChatWin.owner.GetJid("Web"), FaFaChatWin.active.GetJid(resource), fn, fileid);    
...
FaFaChatFileUpload.Accept = function (fn, fnID, ftype, _jid) {
    ...
    FaFaMessage.AcceptFile(FaFaChatWin.owner.GetJid("Web"), FaFaChatWin.active.jid + "/" + resource, fn, fnID);
},
...
FaFaChatFileUpload.ShowFile = function (jid, fileid, path, attr) {
    ...
    if ("jpg,jpeg,bmp,gif,png".indexOf(t.toLocaleLowerCase()) > -1) {
       ...
       $(".webim_stop_aduio").on("click",function(){
           $(this).attr("class","webim_play_aduio");
           $(this).attr("title","单击停止");
       });
       
       $(".webim_play_aduio").on("click",function(){
...
FaFaChatFileUpload.RequestReceive = function (nick, _jid, fileName, fileId, senddate) {
    ...
    if (",jpg,jpeg,bmp,gif,png,".indexOf(fixed.toLocaleLowerCase()) > -1) {
        ...
        if (jidObj.resource == "Web") FaFaMessage.AcceptFile(FaFaChatWin.owner.GetJid(), _jid, fileName, fileId);
        else
        //自动获取图片地址
...
FaFaChatWin.init = function (ownerjid) {
    if (typeof ($.fn.ajaxSubmit) == "undefined") {
        LoadJs.load(WebIM_domain+"/bundles/justsybase/js/jquery.form.js");
    }
    if (typeof (FaFaMessage) == "undefined" || typeof ($.fn.modal) == "undefined") {
        if(typeof ($.fn.modal) == "undefined")
            LoadJs.load(WebIM_domain+"/bundles/justsybase/js/bootstrap.js");
    ...
    if (_cx.length == 0) {
        ...
        this.obj.find(".webim_modal-header .webim_nickname span").on("click", function (){
        ...
        this.obj.find(".webim_modal-header .webim_close").on("click", function () {
           FaFaChatWin.close();
        });
           
        this.obj.find(".webim_modal-header .webim_fafa_chat_photo").on("mouseover", function (){
        ...
        this.obj.find(".webim_modal-header .webim_fafa_chat_photo").on("click", function (){
        ...
        this.obj.find("#fafa_chat_window_close").on("click", function () { FaFaChatWin.close() });
        this.obj.find("#fafa_chat_window_send").on("click", function () { FaFaChatWin.send() });
        this.obj.find(".webim_fafa_chat_hint a").on("click", function () { $(this).parent().hide() });
    ...
    FaFaMessage.GetPresence(function (pre) {
        ...
        if (pre.Type == "hasofflinefile") {
            if (FaFaChatWin.owner.resource.join(",").indexOf("Win")>-1 || FaFaChatWin.owner.resource.join(",").indexOf("Android")>-1) return;
            //收到离线文件、图片
    ...
    FaFaMessage.GetIQ(function (iq) {
        if (iq.tagName == "fafawebfile") {
            ...
            if (action == "cancel")
            {
                if(Jid.Parse(iq.From).resource=="Win")
    ...
    FaFaMessage.GetMessage(function (msg) {
        ...
        //判断 是不否是通过web端发送的消息
        if (msg.From.resource.indexOf("Web") > -1 && text.indexOf("Web") == 0)
          text = text.substring(7);          
...
FaFaChatWin.ShowCard = function () {
    ...
    if (_cx.length == 0) {
        ...
        cx.find(".webim_modal-header .webim_close").on("click", function () { $("#fafa_chat_employee_card").hide() });
  ...
  if (_chatmsg.length==0) {
    ...
    var msg_html="<div class='modal-header webim_chatmsg_header'><div class='webim_chatmsg_title_flag'></div><span class='webim_chatmsg_title_word'>聊天消息管理</span>"+
     ...
     　　 $("#webim_chatmsg_right").text("没有聊天记录！");
      ...
      //点击好友面板
      $("#chatmsg_roster").on("click", function () {
          FaFaChatWin.ShowSearchRoster();
      });
      
      //组织机构
      $("#chatmsg_org").on("click",function(){
      ...
      //系统消息
      $("#chatmsg_systmMsg").on("click",function(){
         $("#chatmsg_contacts").hide();
      ...
      //搜索工具栏
      $("#chatmsg_search").on("click",function(){
         $("#chatmsg_serach_condition").toggle();
      });
      
      //导出聊天记录
      $("#chatmsg_export").on("click",function(){
      ...
      //首页
      $(".webim_chatmsg_first_pager").on("click",function(){
      ...
      //末页
      $(".webim_chatmsg_last_pager").on("click",function(){
      ...
      //上一页
      $(".webim_chatmsg_prev_pager").on("click",function(){
        FaFaChatWin.ShowLoading(1);
      ...
      //下一页
      $(".webim_chatmsg_next_pager").on("click",function(){
        FaFaChatWin.ShowLoading(1);
        ChatMessage.currentPageIndex=ChatMessage.currentPageIndex+1;
        var limit = (ChatMessage.currentPageIndex-1)*ChatMessage.everyPage+","+ChatMessage.currentPageIndex*ChatMessage.everyPage;        
...
FaFaChatWin.ShowGroupCard = function (gid) {
    ...
    if (_cx.length == 0) {
        ...
        _cx.find(".webim_group_header .webim_group_close").on("click", function () { $("#fafa_chat_group_card").hide();});        
...
FaFaChatWin.ShowGroupMember = function(gid){
    ...
    $(".li_groupmember").on("dblclick",function(){
...
FaFaChatWin.DisplayChatMsg = function(data){
   ...
   $(".webim_chatmsg_noselected").on("click",function(){
