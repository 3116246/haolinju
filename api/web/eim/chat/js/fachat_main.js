...
var organ_OnlineIco = FaChatMain_domain +"/eim/chat/images/employee.png";
var organ_OfflineIco = FaChatMain_domain + "/eim/chat/images/employeeoff.png";
var Sound = { Msg:FaChatMain_domain + "/eim/chat/music/NewMsg.wav"};
...
//加载页面需要的资源
LoadCSS.load(FaChatMain_domain + "/eim/chat/css/FaFaChatRoster.css");
LoadCSS.load(FaChatMain_domain + "/eim/chat/css/zTreeStyle.css");
LoadJs.load(FaChatMain_domain + "/eim/chat/js/jquery.jplayer.min.js");
LoadJs.load(FaChatMain_domain + "/eim/chat/js/jquery.ztree.core-3.4.min.js");
LoadJs.load(FaChatMain_domain + "/bundles/justsybase/js/bootstrap.js");
...
FaFaChatMain.init = function () {
    ...
    FaFaMessage.GetPresence(function (pre) {
        ...
        if (pre.Type == "hasofflinefile") {
            ...
            if (FaFaChatMain.owner.resource.join(",").indexOf("Win")>-1
                || FaFaChatMain.owner.resource.join(",").indexOf("Android")>-1) return;
            //将消息发送者存入未读消息列表中
...
FaFaChatMain.LoadGroupInfo = function(){
  	 ...
  	 for(var i=0;i<list.length;i++)
  	 {
        ...
        if (FaFaChatMain.GroupInfo.get(groupid)==null)
           FaFaChatMain.GroupInfo.put(groupid,{"logo":"/bundles/justsybase/images/"+(groupclass=="circlegroup"?"default_circle.png":(groupclass=="meeting"||groupclass=="discussgroup"?"default_meeting.png":"default_group.png")),"groupclass":groupclass,desc:$(list[i]).attr("groupdesc"),post:$(list[i]).attr("grouppost").replace(/＜br＞/g,"<br/>"),isload:false,name:groupname});
