...
ComFileUpload.prototype={
	...
	addFileLoad:function(){
		if($(this.container).find('li').length < this.maxCount){
			if($(this.container).find('li').length>4){
				if($(this.container).find('li').length==5){
			...
			html.push("<input style='width:65px;' name='filedata' type='file' class='upload_file'/></div>");
	...
	addFileLoadSmall:function(){
		if($(this.container).find('li').length < this.maxCount){
			...
			html.push("<input style='width:25px;' type='file' name='filedata' class='upload_file upload_file_small'/></div>");
	...
	registerEvent:function(){
		...
		$("span.upload_abort").live('click',function(){
			...
			li.remove();
			if($(p.container).find('li').length==5){
