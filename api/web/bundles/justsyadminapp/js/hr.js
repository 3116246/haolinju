...
var reportManage = {
	...
	Save:function(){
		...
		if ( $.trim($("#report_title").val())==""){
			this.showhint("请输入报表标题！",this.icon.error);
			return false;
		}
    var staffobj = mb_seluser.getSelValue();
    var file=$("#filename").val();
    var parameter = {"reportid":this.reportid,"title":$.trim($("#report_title").val()),"fileid":this.fileid,"filename":file,"staff":staffobj};
    $.post(this.edit_url,parameter,function(calldata){
    	 if (calldata.success){
    	   reportManage.reportid="";
    	   reportManage.fileid="";
	...
	//删除上传的报表文件
	Delete:function(ev){
		var report_id = $(ev).attr("reportid");
    ...
    showDialog.callback=function(result){
	  	 if(result=="Yes"){
	  	 	 $.post(reportManage.delete_url,{"reportid":report_id,"fileid":file_id },function(data){
	  	 	 	 if (data.success){
	  	 	 	 	 showDialog.Success("提示！","删除报表文件成功！");
  ...
  pageselectCallback:function(page_index){
	...
	fulldata:function(data){ 
  	 ...
  	 if ( data != null && data.length>0){
  		    ...
  		    html.push("   <span style='margin-left:20px;' fileid='"+row.fileid+"' reportid='"+row.id+"' onclick='reportManage.Delete(this);' class='mb_delete_button' title='删除' onclick=''></span></td>");
  		    html.push("</tr>");
 	 	   }
		 }
	  else {
