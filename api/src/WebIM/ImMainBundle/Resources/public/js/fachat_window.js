...
var MessageUtil = {
    faces: {},
    Get: function () {
        $.getJSON(WebIM_domain+"/get/faces?jsoncallback=?", {}, function (data) {
...
FaFaChatFileUpload.Init = function () {
    ...
    var action = WebIM_domain + "/webim/file?c=" + callUrl;
...
FaFaChatWin.init = function (ownerjid) {
            ...
            if (loaded != null) {
                ...
                if (FaFaChatWin.active && loaded.jid == FaFaChatWin.active.jid && FaFaChatWin.active.state != pre.Type) {
                    ...
                    else if (pre.Type == "online")
                    {
                       ...
                       {
                         case "FaFaWin":
                           online_resource ="<div>好友通过<span style='color:red;'>&nbsp;FaFa-PC&nbsp;</span>上线了</div>";
                           break;
                         case "FaFaAndroid":
                           online_resource ="<div>好友通过<span style='color:red;'>&nbsp;FaFa-Android&nbsp;</span>上线了</div>";
                           break;
                         case "FaFaIphone":
                           online_resource ="<div>好友通过<span style='color:red;'>&nbsp;FaFa-Iphone&nbsp;</span>上线了</div>";
                           break;
                         default:
    ...
    FaFaMessage.GetMessage(function (msg) {
        ...
        {
           case "FaFaWin":
             text+="<div style='color:#999999;font-size:11px;padding-left:0px;'>(来自FaFa-PC)</div>";
             break;
           case "FaFaAndroid":
             text+="<div style='color:#999999;font-size:11px;padding-left:0px;'>(来自FaFa-Android)</div>";
             break;
           case "FaFaIPhone":
             text+="<div style='color:#999999;font-size:11px;padding-left:0px;'>(来自FaFa-IPhone)</div>";
             break;
           case "iPad":
             text+="<div style='color:#999999;font-size:11px;padding-left:0px;'>(来自FaFa-iPad)</div>";
             break;
        }
    ...
    var msg_html="<div class='modal-header webim_chatmsg_header'><div class='webim_chatmsg_title_flag'></div><span class='webim_chatmsg_title_word'>聊天消息管理</span>"+
     ...
     　　 $("#webim_chatmsg_right").text("没有聊天记录！");
      ...
      $("#chatmsg_export").live("click",function(){
        var url = WebIM_domain + "/wechatmsg/export?ownerjid="+Jid.Bear(FaFaChatWin.owner.jid)+
...
FaFaChatWin.WriteChatMsg=function (_tojid,_fromjid,_tonick,_fromnick,_date,_msg,_msgtype,_ownerjid,func){
      ...
      if(_fromjid != _ownerjid){
        if(_fromjid.indexOf("FaFaWin")>-1)
           _msg += "<div style='color:#999999;font-size:11px;padding-left:0px;'>(来自FaFa-PC)</div>";
        else if(_fromjid.indexOf("FaFaAndroid")>-1)
           _msg += "<div style='color:#999999;font-size:11px;padding-left:0px;'>(来自手机FaFa-Android)</div>";
        else if(_fromjid.indexOf("FaFaIphone")>-1)
           _msg += "<div style='color:#999999;font-size:11px;padding-left:0px;'>(来自手机FaFa-Iphone)</div>";
        else if(_fromjid.indexOf("Ipad")>-1)
           _msg += "<div style='color:#999999;font-size:11px;padding-left:0px;'>(来自FaFa-iPad)</div>";
      }
      var url = WebIM_domain+ "/wechatmsg/writemsg?jsoncallback=?";
...
FaFaChatWin.searchChatMsg=function(_ownerjid,_otherjid,_date,_content,_msgType,func)
    {
      var url = WebIM_domain +"/wechatmsg/searchmsg?jsoncallback=?";
...
FaFaChatWin.pagingChatMsg=function(_ownerjid,_otherjid,_date,_content,_pageindex,func)
    {
      var url = WebIM_domain +"/wechatmsg/paging?jsoncallback=?";
...
FaFaChatWin.ChatMsgReomveById=function(_ownerjid,_id){
       var url = WebIM_domain + "/wechatmsg/deleteById?jsoncallback=?";
