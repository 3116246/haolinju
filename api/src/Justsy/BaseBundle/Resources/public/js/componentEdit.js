...
var ComponentAttr = {
	...
	save:function()
	{
		...
		var basicAttrXml = this.collecteBasicData();
		if(basicAttrXml!=null)  //设置公共属性，在提交配置 前需要将属性设置添加到组件配置中
			componentObject._para.basicAttr=basicAttrXml;
		componentObject.save();
		AppConfigManager.hint = true;
	},
	collecteBasicData:function()  //收集设置的公共属性配置
	{
		return component_publicstyle.saveToXML();
	},
	cancel:function(){
    	AppConfigManager.refreshInterNode();
