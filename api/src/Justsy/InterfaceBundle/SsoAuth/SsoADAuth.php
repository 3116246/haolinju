...
class SsoADAuth extends Controller implements  ISsoAuth
{
	private $logger = null;
	//ldap认证
	public static function userAuthAction($container,$request,$con,$con_im,$login_account,$password,$comefrom)
	{
        ...
        //有效的配置及格式：{"ENO":"","AD":
        //                             {"HOST":"XXX","DN":"","LIST":
        //                             			{"UID":"uid","NAME":"cn","DEPTNAME":"ou","MOBILE":"mobile"}
        //                             }
        //                   }
        try 
	    {
	        ...
	        $ldapConfgiObject = $ldapConfgiObject["AD"];
	...
	private function loginLdapServer($config,$userid,$pwd)
	{
			...
			$bind = ldap_bind($conn, $userid,$pwd);
			if(!$bind){
