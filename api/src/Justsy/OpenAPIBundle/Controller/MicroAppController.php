...
class MicroAppController extends Controller
{
	...
	public function getPagePathAction($name) {
        ...
        $conn_im = $this->get("we_data_access_im");
        $request = $this->getRequest();
        if(empty($name)) return $this->responseJson('错误：请求参数错误');
		...
		try {
			...
			$data=$conn->GetData('dt',$sql,$para);
			if($data!=null && count($data['dt']['rows'])>0) {
				$re=str_replace("'", "\'", $data['dt']['rows'][0]['msg_content']) ;
			}
		} catch (\Exception $e) {
			$this->get('logger')->err($e->getMessage());
			$re= '错误：'.$e->getMessage();
	...
	public function TextMsgAction() {
		...
		$sql="SELECT b.fafa_jid openid,a.micro_source FROM we_micro_account a LEFT JOIN we_staff b ON b.login_account=a.number AND b.eno=a.eno WHERE a.number=?";
		$para=array($micro_account);
		$data=$conn->GetData('dt',$sql,$para);
        ...
        try {
		    ...
		    if(!empty($fafa_jids)) {
	            ...
	            if(!empty($jids)) $re= $apicontroller->sendMsg2($microOpenid,implode(",",$jids),$msgContent,$msgType,false,"0",$id);
	        }
